...
class NLUModel:
    def __init__(self):
        ...
        self.intents = json.loads(open(os.path.join(os.path.dirname(__file__), 'intents.json', 'intents2.json', 'intents3.json', 'intents4.json', 'intents5.json', 'intents6.json', 'intents7.json', 'intents8.json', 'intents9.json')).read())
        self.words = []
        self.classes = []
        self.documents = []
        self.ignore_letters = ['?', '!', '.', ',', "'s", "'m", "'re", "'ve", "'ll", "'d", "'t", "n't"]
        self.ignore_words = string.punctuation + string.whitespace
        self.ignore_words += string.digits
        self.ignore_words += string.ascii_letters
        self.ignore_words = set(self.ignore_words)
        self.ignore_words = re.compile(f"[{''.join(self.ignore_words)}]")
        self.ignore_words = re.compile(f"[{string.punctuation}]")
        self.ignore_words = re.compile(f"[{string.whitespace}]")
        self.ignore_words = re.compile(f"[{string.digits}]")
        self.ignore_words = re.compile(f"[{string.ascii_letters}]")
        self.ignore_words = re.compile(f"[{string.ascii_letters}]")
        self.ignore_words = re.compile(f"[{string.punctuation}]")
        self.ignore_words = re.compile(f"[{string.whitespace}]")
        self.ignore_words = re.compile(f"[{string.digits}]")
